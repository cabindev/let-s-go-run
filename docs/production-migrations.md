...
| 2569-09-06 | `20260906100829_add_registration_payment_issue` | เพิ่ม **`Registration.paymentIssueAt`** + index — เคสเดียวกันของฝั่งสมัครงานวิ่ง · **ตัวนี้แตะตารางที่มีข้อมูลจริงแล้ว (23 ใบสมัคร)** แต่เป็นการเพิ่มคอลัมน์ nullable อย่างเดียว ไม่แก้/ลบของเดิม ข้อมูลที่มีอยู่จะได้ค่า NULL ทั้งหมด · ยืนยันหลังรัน: ตาราง 9 → 17 · `_prisma_migrations` 21 → 25 แถว |
| _ยังไม่ได้รัน_ | `20260907064924_add_invite_codes` | โควตาสิทธิพิเศษ (สปอนเซอร์/แขกผู้จัดงาน) — เพิ่มตารางใหม่ `InviteCode` **หนึ่งตาราง** และเพิ่ม 2 คอลัมน์ nullable กับ 2 index ใน `Registration` (`inviteCodeId` เป็น FK ไป `InviteCode` แบบ `ON DELETE SET NULL`, `inviteGroupName` เป็น snapshot ชื่อกลุ่ม) · **แตะตารางที่มีข้อมูลจริง** แต่เพิ่มคอลัมน์ nullable อย่างเดียว ใบสมัครเดิมทั้งหมดจะได้ค่า NULL ซึ่งแปลว่า "สมัครแบบปกติ" พอดี · รันในเครื่องแล้วเมื่อ 2569-09-07 · **ต้อง export ฐานข้อมูลก่อนรันบน production ตามกฎในเอกสารนี้** |
