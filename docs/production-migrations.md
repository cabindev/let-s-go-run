...
| _ยังไม่ได้รัน_ | `20260906043730_add_shop` | ระบบร้านค้า — **เพิ่มตารางใหม่ล้วน** (`Product`, `ProductVariant`, `ProductImage`, `Cart`, `CartItem`, `Order`, `OrderItem`, `ShopSetting`) ไม่แก้/ลบคอลัมน์ของตารางเดิมเลย จึงไม่กระทบข้อมูลที่มีอยู่ · รันในเครื่องแล้วเมื่อ 2569-09-06 · **ต้อง export ฐานข้อมูลก่อนรันบน production ตามกฎในเอกสารนี้** |
| _ยังไม่ได้รัน_ | `20260906063736_add_product_image_category` | หมวดรูปสินค้า — เพิ่ม enum `ProductImageCategory` และคอลัมน์ `ProductImage.category` (default `OTHER`) + index แตะเฉพาะตาราง `ProductImage` ที่เพิ่งสร้างในรอบเดียวกัน · รันต่อจาก `add_shop` ในครั้งเดียวได้ |
| _ยังไม่ได้รัน_ | `20260906091438_add_order_payment_issue` | เพิ่ม `Order.paymentIssueAt` + index — ใช้ปักธงเคส "เงินเข้าแต่ออเดอร์หมดอายุ/ถูกยกเลิกไปแล้ว" เพิ่มคอลัมน์อย่างเดียว |
| _ยังไม่ได้รัน_ | `20260906100829_add_registration_payment_issue` | เพิ่ม **`Registration.paymentIssueAt`** + index — เคสเดียวกันของฝั่งสมัครงานวิ่ง · **ตัวนี้แตะตารางที่มีข้อมูลจริงแล้ว (23 ใบสมัคร)** แต่เป็นการเพิ่มคอลัมน์ nullable อย่างเดียว ไม่แก้/ลบของเดิม ข้อมูลที่มีอยู่จะได้ค่า NULL ทั้งหมด |
