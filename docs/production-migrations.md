...
| 2569-09-06 | `20260906043730_add_shop` | ระบบร้านค้า — **เพิ่มตารางใหม่ล้วน** (`Product`, `ProductVariant`, `ProductImage`, `Cart`, `CartItem`, `Order`, `OrderItem`, `ShopSetting`) ไม่แก้/ลบคอลัมน์ของตารางเดิมเลย จึงไม่กระทบข้อมูลที่มีอยู่ · รันในเครื่องแล้วเมื่อ 2569-09-06 · **ต้อง export ฐานข้อมูลก่อนรันบน production ตามกฎในเอกสารนี้** |
| 2569-09-06 | `20260906063736_add_product_image_category` | หมวดรูปสินค้า — เพิ่ม enum `ProductImageCategory` และคอลัมน์ `ProductImage.category` (default `OTHER`) + index แตะเฉพาะตาราง `ProductImage` ที่เพิ่งสร้างในรอบเดียวกัน · รันต่อจาก `add_shop` ในครั้งเดียวได้ |
| 2569-09-06 | `20260906091438_add_order_payment_issue` | เพิ่ม `Order.paymentIssueAt` + index — ใช้ปักธงเคส "เงินเข้าแต่ออเดอร์หมดอายุ/ถูกยกเลิกไปแล้ว" เพิ่มคอลัมน์อย่างเดียว |
| 2569-09-06 | `20260906100829_add_registration_payment_issue` | เพิ่ม **`Registration.paymentIssueAt`** + index — เคสเดียวกันของฝั่งสมัครงานวิ่ง · **ตัวนี้แตะตารางที่มีข้อมูลจริงแล้ว (ข้อมูลจริงอยู่แล้ว)** แต่เป็นการเพิ่มคอลัมน์ nullable อย่างเดียว ไม่แก้/ลบของเดิม ข้อมูลที่มีอยู่จะได้ค่า NULL ทั้งหมด · ยืนยันหลังรัน: ตาราง 9 → 17 · `_prisma_migrations` 21 → 25 แถว |
| 2569-09-07 | `20260907064924_add_invite_codes` | โควตาสิทธิพิเศษ (สปอนเซอร์/แขกผู้จัดงาน) — เพิ่มตารางใหม่ `InviteCode` **หนึ่งตาราง** และเพิ่ม 2 คอลัมน์ nullable กับ 2 index ใน `Registration` (`inviteCodeId` เป็น FK ไป `InviteCode` แบบ `ON DELETE SET NULL`, `inviteGroupName` เป็น snapshot ชื่อกลุ่ม) · **แตะตารางที่มีข้อมูลจริง** แต่เพิ่มคอลัมน์ nullable อย่างเดียว ใบสมัครเดิมได้ค่า NULL ซึ่งแปลว่า "สมัครแบบปกติ" พอดี · ยืนยันหลังรัน: ตาราง 17 → 18 · `_prisma_migrations` 25 → 26 · FK ครบ 2 · ใบสมัคร 3 ใบยังอยู่ครบและเป็น NULL ทั้งหมด · **บทเรียน:** พาสต์ SQL ก้อนยาวใน phpMyAdmin แล้วบรรทัดซ้ำจนได้ `#1060 Duplicate column name` — `CREATE TABLE` ล้มทั้งคำสั่งจึงไม่มีตารางค้าง แต่ `ALTER TABLE` ก่อนหน้าผ่านไปแล้ว ต้องเช็ค `information_schema` ก่อนว่ารันถึงไหน แล้วรันต่อเฉพาะส่วนที่เหลือ ห้ามรันซ้ำทั้งก้อน |
